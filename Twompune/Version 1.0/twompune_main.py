...
def bot_thread(bot_id):
    ...
    while connection_retry_count < max_connection_retries and bot_config.running:
        try:
            ...
            login_delay = random.uniform(2.0, 8.0) + (connection_retry_count * 2)
            print(f"Bot {bot_id}: Waiting {login_delay:.2f} seconds before connecting (attempt {connection_retry_count + 1}/{max_connection_retries})")
            ...
            while bot_config.running:
                try:
                    ...
                    if "too many connections" in error_msg or "rate limit" in error_msg:
                        print(f"Bot {bot_id}: Too many connections detected, waiting 10s before retry...")
                        time.sleep(10)
            ...
            if "too many connections" in error_msg or "rate limit" in error_msg or "429" in error_msg:
                retry_delay = 8 + (connection_retry_count * 3) + random.uniform(1, 5)
                print(f"Bot {bot_id}: TOO MANY CONNECTIONS ERROR - Retrying in {retry_delay:.1f}s (attempt {connection_retry_count}/{max_connection_retries})")
                time.sleep(retry_delay)
            elif "connection" in error_msg or "timeout" in error_msg:
                retry_delay = 5 + (connection_retry_count * 2)
                print(f"Bot {bot_id}: Connection error - Retrying in {retry_delay:.1f}s")
                time.sleep(retry_delay)
            else:
    ...
    if connection_retry_count >= max_connection_retries:
        print(f"Bot {bot_id}: Max retries reached, giving up")
